...
var tripBuilder = (function(tripbuilder) {
  ...
  /**
   * Markup override properties
   */
  let markupOverridden = false;
  let markupOverrideValue = 0;
  let markupOverridePercentage = 0;
  let markupOverrideRetailTotal = 0;
  let markupOverrideReasonType = "";
  let markupOverrideReasonOtherDescription = "";
  ...
  function AddServiceCostOverride(serviceId, overridesObject) {
    ...
    if (_.has(currentService, ["data", "costOverrides"])) {
       ...
       * If an override exists for the type of the override you are trying to add, ensure that the ID is copied
       * to ensure that amount is updated in the DB instead of creating a new override line.
       */
      if (_.includes(["markup-value", "markup-percentage", "sell-price"], overridesObject.fieldType)) {
        for (let index = 0; index < currentService.data.costOverrides.length; index++) {
          if (
            ...
            if (_.has(currentService.data.costOverrides[index], "id")) {
              overridesObject.id = currentService.data.costOverrides[index].id;
            }
  ...
  function SetMarkupOverrideValue(value) {
    markupOverrideValue = value;
  }

  function SetMarkupOverridePercentage(percentage) {
    markupOverridePercentage = percentage;
  }

  function SetMarkupOverrideRetailTotal(retailTotal) {
    markupOverrideRetailTotal = retailTotal;
  }

  function GetMarkupOverrideValue() {
    return markupOverrideValue;
  }

  function GetMarkupOverridePercentage() {
    return markupOverridePercentage;
  }

  function GetMarkupOverrideRetailTotal() {
    return markupOverrideRetailTotal;
  }

  function GetMarkupIsOverridden() {
    return markupOverridden;
  }

  function SetMarkupIsOverridden(isOverridden) {
    markupOverridden = isOverridden;
  }

  function GetMarkupReasonType() {
    return markupOverrideReasonType;
  }

  function SetMarkupReasonType(reasonType) {
    markupOverrideReasonType = reasonType;
  }

  function GetMarkupReasonOtherTypeDescription(reason) {
    return markupOverrideReasonOtherDescription;
  }

  function SetMarkupReasonOtherTypeDescription(description) {
    markupOverrideReasonOtherDescription = description;
  }
    ...
    let newRoomConfig = {
      ...
      addRoom: function(name, adults = 0, children = 0, infants = 0, type, existingId = -1) {
        ...
        let newRoom = {
          ...
          infantPax: [],
          nextPaxId: 0,
          addPax: function(firstName, surname, paxType, age, notes = "", existingId = -1) {
            newRoom.nextPaxId++;
            let newPax = {
              id: newRoom.nextPaxId,
          ...
          getPax: function(paxId) {
            for (let index = 0; index < this.pax.length; index++) {
             /**
             * First check if the system has a system_id property
             *
             */        
              if (this.pax[index].hasOwnProperty("system_id")) {
                if(this.pax[index].system_id == paxId){
        ...
        /**
         * If this is an existing room, assign it to the system_id property
         */
        if (existingId != -1) {
          newRoom.system_id = existingId;
        }
      ...
      getRoom: function(roomId) {
        for (let index = 0; index < this.rooms.length; index++) {
           ...
           *
           */
          if (this.rooms[index].hasOwnProperty("system_id")) {
            if (this.rooms[index].system_id == roomId) {
  ...
  tripbuilder.SetMarkupOverrideValue = function(value) {
    SetMarkupOverrideValue(value);
  };

  tripbuilder.SetMarkupOverridePercentage = function(percentage) {
    SetMarkupOverridePercentage(percentage);
  };

  tripbuilder.SetMarkupOverrideRetailTotal = function(retailTotal) {
    SetMarkupOverrideRetailTotal(retailTotal);
  };

  tripbuilder.GetMarkupOverrideValue = function() {
    return GetMarkupOverrideValue();
  };

  tripbuilder.GetMarkupOverridePercentage = function() {
    return GetMarkupOverridePercentage();
  };

  tripbuilder.GetMarkupOverrideRetailTotal = function() {
    return GetMarkupOverrideRetailTotal();
  };

  tripbuilder.SetMarkupIsOverridden = function(isOverridden) {
    return SetMarkupIsOverridden(isOverridden);
  };
  tripbuilder.MarkupIsOverridden = function() {
    return GetMarkupIsOverridden();
  };

  tripbuilder.GetMarkupReasonType = function() {
    return GetMarkupReasonType();
  };

  tripbuilder.SetMarkupReasonType = function(reasonType) {
    SetMarkupReasonType(reasonType);
  };

  tripbuilder.GetMarkupReasonOtherTypeDescription = function(reason) {
    return GetMarkupReasonOtherTypeDescription();
  };

  tripbuilder.SetMarkupReasonOtherTypeDescription = function(description) {
    return SetMarkupReasonOtherTypeDescription(description);
  };
