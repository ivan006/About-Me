...
$(function() {
  ...
  let packageId = getQueryVariable("id");
  ...
  if (referenceNumber !== false) {
    $.ajax({
      ...
      success: function(existingBooking) {
        ...
        /**
         * If we have loaded a booking but the action is set to "duplicate", then load the booking ready to be duplicated
         * using the new set of dates.
         *
         * If the action is not set to duplicate, this is an existing booking that is ready to be updated.
         */
        ...
        if (existingBooking.hasOwnProperty("markupOverride")) {
          if (existingBooking.markupOverride.markup_override_amount) {
            tripBuilder.SetMarkupIsOverridden(true);
            tripBuilder.SetMarkupOverrideValue(existingBooking.markupOverride.markup_override_amount);
            tripBuilder.SetMarkupOverridePercentage(existingBooking.markupOverride.markup_override_percentage);
            tripBuilder.SetMarkupOverrideRetailTotal(existingBooking.markupOverride.markup_override_total_retail);
            tripBuilder.SetMarkupReasonType(existingBooking.markupOverride.markup_override_reason);
            tripBuilder.SetMarkupReasonOtherTypeDescription(existingBooking.markupOverride.markup_override_reason_text);
          }
        }
  ...
  } else if (builderMode == "packagebuilder" && packageId) {
    $.ajax({
      type: "GET",
      url: "/tripbuilder/editPackage",
      data: {id: packageId},
      dataType: "json",
      success: function(existingPackage) {
        tripBuilder.ClearItinerary();
        let startDate = moment(existingPackage.package.package_start_date);
        let endDate = moment(existingPackage.package.package_end_date);
        let diff = 0;
        let diffBetweenStartDates = 0;
        ...
        BookingState.isExistingPackage = true;
        BookingState.packageId = packageId;
        itineraryForRender = renderPackageForEditing(existingPackage);
        ...
        if (existingPackage.hasOwnProperty("markupOverride")) {
          if (existingPackage.markupOverride.markup_override_amount) {
            tripBuilder.SetMarkupIsOverridden(true);
            tripBuilder.SetMarkupOverrideValue(existingPackage.markupOverride.markup_override_amount);
            tripBuilder.SetMarkupOverridePercentage(existingPackage.markupOverride.markup_override_percentage);
            tripBuilder.SetMarkupOverrideRetailTotal(existingPackage.markupOverride.markup_override_total_retail);
            tripBuilder.SetMarkupReasonType(existingPackage.markupOverride.markup_override_reason);
            tripBuilder.SetMarkupReasonOtherTypeDescription(existingPackage.markupOverride.markup_override_reason_text);
          }
        }
    ...
    function(start, end, label) {
      ...
      if (tripBuilder.Itinerary().length > 0) {
        ...
        for (const i of tripBuilder.Itinerary()) {
          ...
          // moment.js' isBetween function allows for inclusive and exclusive selection.
          // the parameter '[)' means your date range can be within startDate (even if they are the same) but cannot go beyond your endDate.
          // More info: https://momentjscom.readthedocs.io/en/latest/moment/05-query/06-is-between/
          if (
  ...
  $("#add-service").on("click", function(e) {
    ...
    metadata.price_code = UI.currentService.priceCode;
  ...
  $('#tabContent > li > a[data-toggle="tab"]').on("shown.bs.tab", function(e) {
    ...
    // If this is the own arrangement, show the add service button.
    // If this is any other service, check if we have selected a product already, then show the add service button.
    if (activeTab == "own-arrangement") {
      $("#add-service").show();
    } else {
      if (UI.currentService.productId != 0) {
        $("#add-service").show();
      } else {
        $("#add-service").hide();
      }
  ...
  $("#costing-sheet-button").on("click", function(e) {
    ...
    /**
     * If this this is not the packagebuilder, leave the Markup overrides as readonly.
     * This can only be done once the modal has been shown
     */
    if (builderMode != "packagebuilder") {
      $("#costing-sheet-markup-total-markup").prop("disabled", true);
      $("#costing-sheet-markup-total-markup-percentage").prop("disabled", true);
      $("#costing-sheet-markup-total-retail").prop("disabled", true);
      if (tripBuilder.MarkupIsOverridden()) {
        $("#costing-sheet-markup-override-table td > input")
          .removeClass("amount-changed")
          .addClass("amount-changed");
        $("#costing-sheet-markup-override-reason-form").show();
      }
    } else {
      if (tripBuilder.MarkupIsOverridden()) {
        $("#costing-sheet-markup-override-table td > input")
          .removeClass("amount-changed")
          .addClass("amount-changed");
        let currentOverrideType = tripBuilder.GetMarkupReasonType();
        if (currentOverrideType == "pre-negotiated-pcm") {
          $("#cost-sheet-markup-override-reason option[value='pre-negotiated-pcm']").prop("selected", true);
          $("#markup-override-other-reason").hide();
        } else {
          let currenOverrideReasonText = tripBuilder.GetMarkupReasonOtherTypeDescription();
          $("#cost-sheet-markup-override-reason option[value='other']").prop("selected", true);
          $("#markup-override-other-reason").val(currenOverrideReasonText);
          $("#markup-override-other-reason").show();
        }
      }
    }
  });
  ...
  $("#save-pax-config-button").on("click", function(e) {
    ...
    // Create a clone of the blank party rooms
    tripInfo.partyRooms = Object.assign({}, blankPartyRooms);
    ...
    tripInfo.partyRooms.family = tripInfo.partyRooms.family.toString();
    tripBuilder.AddTripInfo(tripInfo);
    renderTripInfo();
  ...
  $("#trip-days").on("click", ".g-d-inline > .modal-add-event-trigger", function(event) {
    // Ensure that the "Add service" button is hidden when you open the modal window.
    $("#add-service").hide();
    const agent = tripBuilder.GetAgent();
    const roomConfigCount = tripBuilder.GetRoomConfigCount();
    UI.activeTab = "accommodation";
    if (!agent) {
      alert(
        "Please select an agent before adding services to a booking.\r\nYou can select an agent by editing the trip information."
      );
  ...
  $("#trip-days").on(
    ...
    function(event) {
      ...
      /**
     * If this service has an ID attribute, this is an existing service in the database.
     */
     let existingService = false;
     let serviceMessage = "No version of this service has been saved to the database yet";
     let serviceStateClass = "label-warning";
     if(currentService.data.hasOwnProperty("id"))
     {
       existingService = true;
       serviceMessage = str.concat("A version of this service has been saved to the database and has the ID of ", currentService.data.id);
       serviceStateClass = "label-success";
     }

     $("#modal-edit-" + currentService.serviceType + "-service-state").addClass(serviceStateClass);
     $("#modal-edit-" + currentService.serviceType + "-service-state").html(serviceMessage);
      ...
      let serviceLocation =
        ...
        " " +
        currentService.data.supplier.place.town;
        let selectedStatus = new SlimSelect({
          select: "#edit-" + UI.activeTab + "-service-status"
        });
        selectedStatus.set(bookingStatus);
      switch (currentService.serviceType) {
        ...
        case "own-arrangement":
          dateRange = $("#edit-own-arrangement-pick-up-drop-off").data("daterangepicker");
          dateRange.setStartDate(moment(currentService.data.pick_up_date).toDate());
          dateRange.setEndDate(moment(currentService.data.drop_off_date).toDate());
          $("#edit-own-arrangement-description").val(currentService.data.serviceDescription);
          renderServiceLineStatus("#edit-own-arrangement-service-line-status", bookingStatus);
          break;
        default:
          break;
      }
      /**
       * If you're opening the own arrangement popup modal, there is no need to calculate cost breakdown.
       */
      if (currentService.serviceType == "own-arrangement") return;
      ...
      for (const costing of currentService.data.costing) {
        for (const room of costing.rooms) {
          let roomId = room.hasOwnProperty("roomId") ? room.roomId : room.id;
          if (costRooms.indexOf(roomId) == -1) {
            costRooms.push(parseInt(roomId));
          }
          costPaxRoomCosts.push({roomId: roomId, paxId: room.paxId});
        }
      }
  ...
  */
  ...
  $(".modal-edit-save").on("click", function(e) {
    ...
    let oldServiceDayCount = oldServiceEndDate.diff(oldServiceStartDate, "days");
    if(oldServiceDayCount == 0)
    {
      oldServiceDayCount = 1;
    }
    ...
    let newServiceDayCount = serviceEndDate.diff(serviceStartDate, "days");
    if(newServiceDayCount == 0)
    {
      newServiceDayCount = 1;
    }
    ...
    let costingArray = [];
    if (currentServiceType != "own-arrangement") {
      /**
       * If this service has an id assigned to it already, it means that it is an existing service
       * which means it has existing pax and room config which need to be used when updating the cost breakdown.
       */
      let isBackendUpdate = currentService.data.hasOwnProperty("id") ? true : false;
      costingArray = mapCostBreakDown(isBackendUpdate);
    }
    ...
    metadata.price_code = currentService.data.price_code;

    /**
     * If the service we editing has existing costOverrides (either saved or unsaved in the database),
     * compare the number of days for the existing date range to the number of days for
     * the new date range and if they are not the same, update the rates for the service,
     * which will also remove the cost override since it will no longer be valid for the service.
     */
    if(currentService.data.hasOwnProperty("costOverrides"))
    {
      if(oldServiceDayCount == newServiceDayCount)
      {
        metadata.costOverrides = currentService.data.costOverrides;
      }
    }

    if (currentServiceType == "own-arrangement") {
      let updatedServiceDescription = $("#edit-own-arrangement-description").val();
      metadata.serviceDescription = updatedServiceDescription;
    }
    ...
    $("#modal-edit-" + currentServiceType).modal("hide");
    UI.currentCostPax = mobx.observable.array([], {deep: false});
    // UI.clearCurrentService();
    CURRENT_SERVICE_ID = {};
  });
        ...
        $(".select-service-selector").on("click", function(e) {
          ...
          if (typeof tripInfo == "object") {
            ...
            if (typeof products == "object") {
              ...
              UI.clearCurrentService();
              try {
                if (typeof currentProduct.rates === "undefined") {
                  let errorMessage = generateProductSelectionError(currentProduct);
                ...
                UI.currentService.priceCode = currentProduct.rates_config.priceCode;
                UI.currentService.address =
                ...
                showErrorModal("Error occurred", "This happened at this point", exception);
              }
            }
          }
        });
  ...
  $("#save-booking").on("click", function(e) {
    ...
    for (const room of roomArray) {
      ...
      if (room.hasOwnProperty("system_id")) {
        updatedRoom.system_id = room.system_id;
      }
      rooms.push(updatedRoom);
    }

    data.roomConfig = rooms;
    data.finalTotals = calculateFinalTotals();
    ...
    if (BookingState.isExistingBooking == true) {
      $.ajax({
        ...
        success: function(response) {
          ...
          let bookingReference = getQueryVariable("ref");
          window.location.href = window.location.pathname + "?ref=" + bookingReference;
        }
      });
    } else {
      ...
      $.ajax({
        ...
        success: function(response) {
          ...
          $("#edit-saved-booking-tripbuilder").attr("href", "/tripbuilder?ref=" + response.booking_reference);
  ...
  $("#save-package").on("click", function() {
    ...
    data.budget = tripBuilder.GetTotalBudget();
    data.tripInfo.markup_percentage = tripBuilder.GetMarkUp();
    data.tripInfo.agent = tripBuilder.GetAgent();
    data.package_id = getQueryVariable("id");
    data.packageStatus = tripBuilder.GetBookingStatus();
    let roomConfig = tripBuilder.GetRoomConfig(1);
    let roomArray = roomConfig.getAllRooms();
    let rooms = [];
    ...
    if (tripBuilder.MarkupIsOverridden()) {
      /**
       *
       * $packageCosting['markup_override_amount'] = $markupOverride['markupOverrideAmount'];
       * $packageCosting['markup_override_percentage'] = $markupOverride['markupOverridePercentage'];
       * $packageCosting['markup_override_reason'] = $markupOverride['markupOverrideReason'];
       * $packageCosting['markup_override_reason_tex'] = $markupOverride['markupOverrideReasonText'];
       * $packageCosting['markup_override_total_retail'] = $markupOverride['markupOverrideTotalRetail'];
       */

      let markupOverride = {
        markupOverrideAmount: tripBuilder.GetMarkupOverrideValue(),
        markupOverridePercentage: tripBuilder.GetMarkupOverridePercentage(),
        markupOverrideTotalRetail: tripBuilder.GetMarkupOverrideRetailTotal(),
        markupOverrideReason: tripBuilder.GetMarkupReasonType(),
        markupOverrideReasonText: tripBuilder.GetMarkupReasonOtherTypeDescription()
      };

      data.markupOverride = markupOverride;
    }

    if (BookingState.isExistingPackage == true) {
      $.ajax({
        type: "post",
        url: "/tripbuilder/updatePackage",
        data: data,
        dataType: "json",
        success: function(response) {
          alert("Package has been updated!");
          let packageId = getQueryVariable("id");
          window.location.href = window.location.pathname + "?mode=packagebuilder&id=" + packageId;
        }
      });
    } else {
      ...
      $.ajax({
        ...
        success: function(response) {
          ...
          $("#saving-package-results-url").attr("href", "/tripbuilder/packages/view/" + response.package_id);
  ...
  $(document).on("change", "#markup-calculation-table input[type='text']", function() {
    ...
    if (fieldType == "cost-value") {
                            ...
                            <div class="col-md-6">
                              ...
                              <div class="input-group" style="margin-left: -4px;">
                                  ...
                                  <select name="" class="form-control" id="cost-overridden-reason" style="margin-left: -5px;">
                                      ...
                                      <option value="Zero Cost Override">Zero Cost Override</option>
                                      <option value="Over charging">Over charging</option>
                                      <option value="Under charging">Under charging</option>
                                      <option value="Vat Amendment">Vat Amendment</option>
      ...
      let tempMarkUpValue = parseFloat($(this).val());
      oldValue = BookingState.costingTotals[service].markupAmount;
      newSellValue = parseFloat(closestTen(costValue + tempMarkUpValue));
      markupValue = parseFloat(newSellValue-costValue);
      markupPercentage = costValue == 0 ? 0 : ((newSellValue - costValue) / costValue) * 100;
      newValue = markupValue;
    } else if (fieldType == "markup-percentage") {
      let tempMarkupPercentage = parseFloat($(this).val());
      oldValue = BookingState.costingTotals[service].tempMarkupPercentage;
      newSellValue = costValue == 0 ? 0 : parseFloat(closestTen(costValue + (costValue * tempMarkupPercentage) / 100));
      markupValue = parseFloat(newSellValue - costValue);
      markupPercentage = costValue == 0 ? 0 : ((newSellValue - costValue) / costValue) * 100;
      newValue = markupPercentage;
    } else if (fieldType == "sell-price") {
      let tempSellValue = parseFloat($(this).val());
      oldValue = BookingState.costingTotals[service].sellAmount;
      newValue = closestTen(tempSellValue);
      newSellValue = parseFloat(newValue)
      markupValue = costValue == 0 ? 0 : parseFloat(newValue - costValue);
      markupPercentage = costValue == 0 ? 0 : ((newValue - costValue) / costValue) * 100;
    }
    ...
    $("#costing-sheet-markup-total-cost").html(totalCost.toFixed(2));
    $("#costing-sheet-markup-total-cost").data("total-cost", totalCost.toFixed(2));
    /**
     * If a markup override has been set, update the amounts to reflect the change in costs.
     */
    if (builderMode == "packagebuilder") {
      if (tripBuilder.MarkupIsOverridden()) {
        let retailTotal = tripBuilder.GetMarkupOverrideRetailTotal();
        let markupOverrideValue = retailTotal - totalCost;
        let markupOverridePercentage = totalCost == 0 ? 0 : ((retailTotal - totalCost) / totalCost) * 100;

        tripBuilder.SetMarkupOverrideValue(markupOverrideValue);
        tripBuilder.SetMarkupOverridePercentage(markupOverridePercentage);

        $("#costing-sheet-markup-total-markup").val(markupOverrideValue.toFixed(2));
        $("#costing-sheet-markup-total-markup-percentage").val(markupOverridePercentage.toFixed(2));

        totalMarkup = markupOverrideValue;
        totalMarkupPercentage = markupOverridePercentage;
      } else {
        $("#costing-sheet-markup-total-markup").val(totalMarkup.toFixed(2));
        $("#costing-sheet-markup-total-markup-percentage").val(totalMarkupPercentage.toFixed(2));
        $("#costing-sheet-markup-total-retail").val(totalSell.toFixed(2));
      }
    } else {
  ...
  $(document).on("change", "#costing-sheet-markup-overrides input.markup-override-field", function(e) {
    /**
     * x: Cater for changes in the service costs.
     * x: Update the Totals well.
     * x: change the colour of the fields
     * x: show reason for override
     * x: load overrides a second time.
     */
    let fieldType = $(this).data("field");
    let totalCost = parseInt($("#costing-sheet-markup-total-cost").data("total-cost"));
    let newRetailTotal = 0;
    let markupOverrideValue = 0;
    let markupOverridePercentage = 0;

    if (fieldType == "markup-override-value") {
      let currentRetailTotal = parseFloat($("#costing-sheet-markup-total-retail").val());
      markupOverrideValue = parseFloat($(this).val());
      markupOverrideValue = isNaN(markupOverrideValue) ? 0 : markupOverrideValue;
      newRetailTotal = totalCost + markupOverrideValue;
      markupOverridePercentage = totalCost == 0 ? 0 : ((newRetailTotal - totalCost) / totalCost) * 100;
    } else if (fieldType == "markup-override-percentage") {
      markupOverridePercentage = parseFloat($(this).val());
      markupOverridePercentage = isNaN(markupOverridePercentage) ? 0 : markupOverridePercentage;
      newRetailTotal = parseFloat(totalCost + (totalCost * markupOverridePercentage) / 100);
      markupOverrideValue = newRetailTotal - totalCost;
    } else if (fieldType == "markup-override-total-retail") {
      newRetailTotal = parseFloat($(this).val());
      newRetailTotal = isNaN(newRetailTotal) ? 0 : newRetailTotal;
      markupOverrideValue = newRetailTotal - totalCost;
      markupOverridePercentage = totalCost == 0 ? 0 : ((newRetailTotal - totalCost) / totalCost) * 100;
    }
    markupOverridePercentage = isNaN(markupOverridePercentage) ? 0 : markupOverridePercentage;
    $("#costing-sheet-markup-total-markup").val(markupOverrideValue.toFixed(2));
    $("#costing-sheet-markup-total-markup-percentage").val(markupOverridePercentage.toFixed(2));
    $("#costing-sheet-markup-total-retail").val(newRetailTotal.toFixed(2));
    $("#costing-sheet-total-markup").text(markupOverrideValue.toFixed(2));
    $("#costing-sheet-total-markup-percentage").text(markupOverridePercentage.toFixed(2));
    $("#costing-sheet-total-retail").text(newRetailTotal.toFixed(2));
    tripBuilder.SetMarkupOverrideValue(markupOverrideValue);
    tripBuilder.SetMarkupOverridePercentage(markupOverridePercentage);
    tripBuilder.SetMarkupOverrideRetailTotal(newRetailTotal);
    tripBuilder.SetMarkupIsOverridden(true);
    $("#costing-sheet-markup-override-table td > input")
      .removeClass("amount-changed")
      .addClass("amount-changed");
    if ($("#costing-sheet-markup-override-reason-form").is(":hidden")) {
      $("#costing-sheet-markup-override-reason-form").show();
    }
  });

  $(document).on("change", "#cost-sheet-markup-override-reason", function(e) {
    e.preventDefault();
  ...
  $(document).on("click", "#markup-override-reason-save", function(e) {
    e.preventDefault();
    let reason = $("#cost-sheet-markup-override-reason").val();
    let otherReason = null;
    if (reason == null) {
      alert("Please select a reason for overriding the markups");
      return;
    } else if (reason == "other") {
      otherReason = $("#markup-override-other-reason").val();
      if (otherReason == "") {
        alert("Please enter in a reason for the markup override");
        return;
      }
    }
    if (reason == "other") {
      tripBuilder.SetMarkupReasonOtherTypeDescription(otherReason);
    } else {
      tripBuilder.SetMarkupReasonType(reason);
      $("#markup-override-other-reason").val("");
      // Reset the "other" description just in case.
      tripBuilder.SetMarkupReasonOtherTypeDescription("");
    }
    $("#reason-updated-note")
      .fadeIn()
      .delay(2000)
  ...
  $(document).on("change", "#costing-sheet-markup-total-markup", function(e) {});
    ...
    if (typeof tripInfo == "object") {
      ...
      if (typeof products == "object") {
        try {
          ...
          showErrorModal("Error occurred", "The following error occurred", exception);
        }
      }
    }
  });
  ...
  if (!moment(startDate).isSame(endDate, "day")) {
    ...
    if (Math.ceil(dateDifference) >= 1) {
      ...
      while (currentDate.isSameOrBefore(endDate, "day")) {
        ...
        let multiDayIcons = ["car-rental", "accommodation"]
        let isIcon = multiDayIcons.indexOf(serviceType) >= 0
        let position = isIcon && !isLastDay? "top" : "bottom"
        let endService = createServiceTag(serviceId, serviceType, multiDayHeight, 0, true, true, isLastDay, isIcon);
        $('.g-d-inline[data-day="' + currentDay + '"] > .modal-add-event-trigger > .multiday-holder-'+position).append(
          endService
        );
...
function createServiceTag(
  ...
  if (isIcon == true && !isLastDay) {
    ...
    element.classList.add(`g-multi-day-icon-${serviceType}`);
    element.style.height = "30px";
    element.style.width = "33%";
    element.style.float = "left"
    element.style.display = "inline"
    let icon = document.createElement("img");
    icon.src = `application/modules/ecom_tripbuilder/assets/icons/tripbuilder-multi-day-icon-${serviceType}.png`;
    icon.title = currentService.data.supplier.org_name;
...
/**
 * Render the tripbuilder calendar view
 * @param string existingBooking Reference number of existing booking.
 * @param int diffBetweenStartDates difference in number of days between the start of the existing booking and the new one.
 */
function renderDuplicateBooking(existingBooking, diffBetweenStartDates) {
  let newItinerary = [];
  for (var itinerary of existingBooking.itinerary) {
    // Create clone of itinerary
    ...
    if (diffBetweenStartDates >= 0) {
      newStartDate.add(diffBetweenStartDates, "d");
      newEndDate.add(diffBetweenStartDates, "d");
    } else {
      let positiveDateDiff = diffBetweenStartDates * -1;
      newStartDate.subtract(positiveDateDiff, "d");
      newEndDate.subtract(positiveDateDiff, "d");
    }
  ...
  for (const room of existingBooking.rooms) {
    ...
    let newRoom = currentRoomConfig.getRoom(newRoomId);
    for (const pax of room.pax) {
      newRoom.addPax(pax.name, "", pax.type, pax.age, pax.notes);
    }
  }
...
/**
 * Render the tripbuilder calendar view with a new booking based on a existing booking.
 * @param string existingBooking Reference number of existing booking.
 */
function renderBookingForEditing(existingBooking) {
  ...
  for (const room of existingBooking.rooms) {
    ...
    let newRoom = currentRoomConfig.getRoom(newRoomId);
    for (const pax of room.pax) {
      newRoom.addPax(pax.name, "", pax.type, pax.age, pax.notes, pax.id);
    }
  }
...
/**
 * Render the tripbuilder calendar view with an existing package.
 * @param object Object that represents
 */
function renderPackageForEditing(existingPackage) {
  let newItinerary = [];
  for (var itinerary of existingPackage.services) {
    // Create clone of itinerary
    let newObject = {...itinerary};
    let newStartDate = moment(itinerary.pick_up_date, "YYYY-MM-DD HH:mm:ss").clone();
    let metadata = createServiceMetaData(newObject);
  ...
  tripBuilder.AddRoomConfig(existingPackage.package.package_name);
  // Look at adding package notes as well?
  // tripBuilder.SetBookingNotes(existingPackage.package.notes);
  let currentRoomConfig = tripBuilder.GetRoomConfig(1);

  for (const room of existingPackage.rooms) {
    const newRoomId = currentRoomConfig.addRoom(
      room.room_name,
      room.adults,
      room.children,
  ...
  tripInfo.partyName = existingPackage.package.package_name;
  /**
   * Cast the party rooms numbers before they are displayed on the UI.
   */
  tripInfo.partyRooms.singles = tripInfo.partyRooms.singles.toString();
  tripInfo.partyRooms.doubles = tripInfo.partyRooms.doubles.toString();
  tripInfo.partyRooms.twins = tripInfo.partyRooms.twins.toString();
  ...
  tripInfo.partyRooms.family = tripInfo.partyRooms.family.toString();
  tripBuilder.AddTripInfo(tripInfo);
  tripBuilder.SetAgent(existingPackage.package.agent);
  tripBuilder.SetBookingStatus(existingPackage.package.package_status);
  tripBuilder.SetMarkup(existingPackage.package.markup_percentage);
  let validDateRange = $("#package-valid-date-range").data("daterangepicker");
  let validFromDate = moment(existingPackage.package.valid_from_date, dateFormat);
  let validToDate = moment(existingPackage.package.valid_to_date, dateFormat);
  validDateRange.setStartDate(validFromDate);
  validDateRange.setEndDate(validToDate);
  tripBuilder.SetValidDateRange(validDateRange);
  tripBuilder.SetPriceCode(existingPackage.package.price_code);
  renderTripInfo();
  return newItinerary;
}
...
function calculateRoomRate(room, rates, days) {
  ...
  if (room.type == "double") {
    ...
    cost = calculateTripleRoomRate(room, rates[0], "AD", days, adultPax, childrenPax, infantPax);
  } else if (room.type == "family") {
    cost = calculateFamilyRoomRate(room, rates[0], "AD", days, adultPax, childrenPax, infantPax);
  } else {
    cost = calculateDoubleRoomRate(room, rates[0], "AD", days, adultPax, childrenPax, infantPax);
  }

  // if (room.hasOwnProperty("system_id")) {
  //   cost.id = room.system_id;
  // }
  return cost;
}
    ...
    for (const pax of paxArray) {
      ...
      switch (pax.pax.type) {
        case "adult":
          ...
          unitCostPrice = adultCostPrice;
          unitSellPrice = adultPrice;
          break;
        case "child":
          paxServiceTotal = childrenPrice;
          paxServiceCostTotal = childrenCostPrice;
          unitCostPrice = childrenCostPrice;
          unitSellPrice = childrenPrice;
          break;
        case "infant":
          paxServiceTotal = infantPrice;
          paxServiceCostTotal = infantCostPrice;
          unitCostPrice = infantCostPrice;
          unitSellPrice = infantPrice;
          break;
        default:
          paxServiceTotal = adultPrice;
          paxServiceCostTotal = adultCostPrice;
      ...
      /**
       * Entrance fees are a unique instance of a service where
       * it could take place over one or many days.
       */
      if (UI.activeTab == "entrance-fee") {
        paxServiceTotal = paxServiceTotal * days;
        paxServiceCostTotal = paxServiceCostTotal * days;
      }

      let newCostBreakdown = {
        pax: pax.pax,
        paxId: pax.pax.id,
        roomId: pax.roomId,
        rooms: [{ roomId: pax.roomId, paxId: pax.pax.id }],
        unitCost: unitCostPrice,
        unitSell: unitSellPrice,
      ...
      // if (breakdown.hasOwnProperty("id")) {
      //   costing.id = breakdown.id;
      // }
      costBreakDown.push(newCostBreakdown);
    }
  } else {
    let paxServiceTotal = 0;
    let paxServiceCostTotal = 0;
...
function updateExistingServiceRate(totalPax, roomId, paxId, paxType, paxName, rates, days, description) {
  let costBreakDown = [];
  ...
  if (maxPaxPerFcu == 1) {
      ...
      let newCostBreakdown = {
        paxId: paxId,
        roomId: roomId,
        rooms: [{ roomId: roomId, paxId: paxId }],
        unitCost: unitCostPrice,
        unitSell: unitSellPrice,
        desc: `${paxName} - ${description}`,
        sellRate: paxServiceTotal,
        costRate: paxServiceCostTotal,
        agentRate: paxServiceTotal,
        total: paxServiceTotal
...
function calculateDoubleRoomRate(room, rates, ageCategory = "AD", days, adultPax = 0, childrenPax = 0, infantPax = 0) {
  ...
  if (totalPax == 1) {
    ...
    costString = `<strong>${room.name}</strong>: ${room.type} room at ${sellRate} x ${days} nights`;
  } else if (totalPax == 2) {
...
function calculateTripleRoomRate(room, rates, ageCategory = "AD", days, adultPax = 0, childrenPax = 0, infantPax = 0) {
  let costBreakDown = {};
  let totalRoomPrice = 0;
  let sellRate = 0;
  let costRate = 0;
  ...
  sellRate = parseFloat(adultSellRateType.tr);
  costRate = parseFloat(adultCostRateType.tr);
  costString = `<strong>${room.name}</strong>: Triple room at ${sellRate} x ${days} nights`;
...
function calculateFamilyRoomRate(room, rates, ageCategory = "AD", days, adultPax = 0, childrenPax = 0, infantPax = 0) {
  let costBreakDown = {};
  let totalRoomPrice = 0;
  let sellRate = 0;
  let costRate = 0;
  ...
  sellRate = parseFloat(adultSellRateType.qr);
  costRate = parseFloat(adultCostRateType.qr);
  costString = `<strong>${room.name}</strong>: Family room at ${sellRate} x ${days} nights`;
...
function createCostingTotalsObject() {
  /**
   * Calculate the total costs/sell/agent prices
   */
  let totalCost = BookingState.costingTotals
  ...
  return {
    ...
    totalMarkupPercentage: totalMarkupPercentage
  };
}
/**
...
function renderCostingSheet() {
  ...
  if (tripBuilder.MarkupIsOverridden()) {
    totalSell = tripBuilder.GetMarkupOverrideRetailTotal();
    totalAgent = tripBuilder.GetMarkupOverrideRetailTotal();
    totalMarkup = tripBuilder.GetMarkupOverrideValue();
    totalMarkupPercentage = tripBuilder.GetMarkupOverridePercentage();
  }
  ...
  let markupOverridesSource = document.getElementById("costing-sheet-markup-override-template").innerHTML;
  let markupOverrideTemplate = Handlebars.compile(markupOverridesSource);
  let markupOverrideTemplateData = {
    totalCost: totalCost.toFixed(2),
    totalSell: totalSell.toFixed(2),
    totalAgent: totalAgent.toFixed(2),
    totalMarkup: totalMarkup.toFixed(2),
  ...
  /**
   * If this is not the packgebuilder, only show the reason for the override, but not the form to edit it.
   */
  let showReason = builderMode != "packagebuilder" ? true : false;
  markupOverrideTemplateData.showReason = showReason;
  if (showReason) {
    let reasonType = tripBuilder.GetMarkupReasonType();
    markupOverrideTemplateData.type = reasonType;
    if (reasonType == "other") {
      markupOverrideTemplateData.description = tripBuilder.GetMarkupReasonOtherTypeDescription();
    }
  } else {
  }

  let markupsHtml = markupOverrideTemplate(markupOverrideTemplateData);

  $("#costing-sheet-wrapper").html(html);
  $("#costing-sheet-totals").html(totalsHtml);
  $("#costing-sheet-markup-overrides").html(markupsHtml);
  ...
  if (UI.activeTab == "accommodation") {
    ...
    for (let room of roomArray) {
      if (UI.currentService.rates == null) {
        ...
        let newCosts = calculateRoomRate(room, UI.currentService.rates, numberOfDays);
        if (room.hasOwnProperty("system_id") && isEdit == true) {
          let currentService = tripBuilder.GetService(CURRENT_SERVICE_ID);
          if (currentService.data.hasOwnProperty("id"))
          {
            if (currentService.data.hasOwnProperty("costing")) {
              for (let costing of currentService.data.costing) {
                for (const costingRoom of costing.rooms) {
                  if (costingRoom.roomId == room.system_id) {
                    costing.agentRate = newCosts.agentRate;
                    costing.costRate = newCosts.costRate;
                    costing.sellRate = newCosts.sellRate;
                    costing.desc = newCosts.desc;
                    costing.unitCost = newCosts.unitCost;
                    costing.unitSell = newCosts.unitSell;
                    costing.total = newCosts.total;
                    costArray.push(costing);
                    break;
                  }
                }
              }
    ...
    let currentService = tripBuilder.GetService(CURRENT_SERVICE_ID);
    // maybe look at using && isEdit == true??
    // Refactor some of this code, find a way to remove duplicate functionality.
    // Test out car hire.
    if(typeof currentService !== "undefined")
    {
      if (currentService.data.hasOwnProperty("id")) {
        if (currentService.data.hasOwnProperty("costing")) {
          /**
           * In order to recalculate the cost for an existing service requires
           * iterating
           */
          let totalPax = _.sumBy(currentService.data.costing, "totalPax");
          for (let currentCosting of currentService.data.costing) {
            if (currentCosting.totalPax == 1) {
              let currentRoom = roomConfig.getRoom(currentCosting.rooms[0].roomId);
              let currentPax = currentRoom.getPax(currentCosting.rooms[0].paxId);
              let updatedCost = updateExistingServiceRate(totalPax, currentCosting.rooms[0].roomId, currentCosting.rooms[0].paxId, currentPax.type, currentPax.name, UI.currentService.rates, numberOfDays, UI.currentService.productDescription);
              currentCosting.agentRate = updatedCost.agentRate;
              currentCosting.costRate = updatedCost.costRate;
              currentCosting.sellRate = updatedCost.sellRate;
              currentCosting.total = updatedCost.total;
              currentCosting.unitCost = updatedCost.unitCost;
              currentCosting.unitSell = updatedCost.unitSell;
              currentCosting.desc = updatedCost.desc;
              updatedCost.rooms = currentCosting.rooms;
              costArray.push(currentCosting);
            } else {
              let updatedCost = updateExistingServiceRate(totalPax, null, null, null, null, UI.currentService.rates, numberOfDays, UI.currentService.productDescription);
              currentCosting.agentRate = updatedCost.agentRate;
              currentCosting.costRate = updatedCost.costRate;
              currentCosting.sellRate = updatedCost.sellRate;
              currentCosting.total = updatedCost.total;
...
function calculateFinalTotals() {
  ...
  if (tripBuilder.MarkupIsOverridden()) {
    finalTotals.markupOverrideAmount = tripBuilder.GetMarkupOverrideValue();
    finalTotals.markupOverridePercentage = tripBuilder.GetMarkupOverridePercentage();
    finalTotals.markupOverrideTotalRetail = tripBuilder.GetMarkupOverrideRetailTotal();
    finalTotals.markupOverrideReason = tripBuilder.GetMarkupReasonType();
    finalTotals.markupOverrideReasonText = tripBuilder.GetMarkupReasonOtherTypeDescription();
  } else {
    finalTotals.markupOverrideAmount = "";
    finalTotals.markupOverridePercentage = "";
    finalTotals.markupOverrideTotalRetail = "";
    finalTotals.markupOverrideReason = "";
    finalTotals.markupOverrideReasonText = "";
  }
  return finalTotals;
}
...
function mapCostBreakDown(isUpdate = false) {
  ...
  for (const breakdown of breakdowns) {
     ...
     */
    if (UI.activeTab == "accommodation" || UI.activeTab == "car-rental") {
      let roomsIncluded = [];
      for (const room of breakdown.rooms) {
        let currentRoomId = room.hasOwnProperty("roomId") ? room.roomId : room.id;
        let currentRoom = currentRoomConfig.getRoom(currentRoomId);
        if (!roomsIncluded.includes(currentRoomId)) {
          costing.totalPax += currentRoom.adults + currentRoom.children + currentRoom.infants;
          roomsIncluded.push(currentRoomId);
        }
        if (isUpdate) {
          costing.rooms = breakdown.rooms;
        } else {
          for (const pax of currentRoom.pax) {
            if (costing.rooms.find(room => room.roomId == currentRoomId && room.paxId == pax.id) === undefined) {
              costing.rooms.push({
...
function handleServiceDateChange(ev, picker) {
  ...
  /**
   * If we are editing an existing service (either saved or unsaved in the database),
   * compare the number of days for the existing date range to the number of days for
   * the new date range and if they are not the same, update the rates for the service,
   * which will also remove the cost override since it will no longer be valid for the service.
   */
  if(isEdit)
  {
    // Get existing services start and end dates
    let currentService = tripBuilder.GetService(CURRENT_SERVICE_ID);
    let existingServiceStartDate = moment(currentService.startDate);
    let existingServiceEndDate = moment(currentService.endDate);

    // Calculate the number of days for the service
    let existingNumberOfDays = existingServiceEndDate.diff(existingServiceStartDate, "days");
    if (existingNumberOfDays == 0) {
      existingNumberOfDays = 1;
    }
    // Get updated services start and end dates
    // and calculate the new number of days for the service.
    let updatedNumberOfDays = serviceEndDate.diff(serviceStartDate, "days");
    if (updatedNumberOfDays == 0) {
      updatedNumberOfDays = 1;
    }
    if(existingNumberOfDays != updatedNumberOfDays){
...
function generateProductSelectionError(currentProduct) {
  let errorMessage = `Rates are missing or incorrect for the following product:<br>
   Product name: ${currentProduct.name}<br>
   Product code: ${currentProduct.option.CODE}<br>
   Supplier: ${currentProduct.option.SUPPLIER}`;
  return errorMessage;
}
function showErrorModal(title, message, error) {
  $("#error-message").html(error.message);
  $("#error-stack-trace").html(error.stack);
  $("#error-modal").modal("show");
}
closestTen = (e) => {
  return Math.ceil(parseFloat(e) / 10) * 10;
}
